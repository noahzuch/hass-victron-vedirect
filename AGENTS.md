...
- **Repository:** noahzuch/hass-victron-vedirect

**Key directories:**

- `custom_components/victron_vedirect/` - Main integration code
...
- Integration logs: `custom_components.victron_vedirect: debug` in `config/configuration.yaml`
...
- Use the domain `victron_vedirect` for all DOMAIN references
- Prefix all integration-specific classes with `VictronVEDirect`
- Use "Victron VE.Direct" as the display title
...
- Inherit from platform base + `VictronVEDirectEntity`
...
- `tests/` mirrors `custom_components/victron_vedirect/` structure
