...
This document describes all configuration options and settings available in the Victron VE.Direct custom integration.
...
### `victron_vedirect.example_service`
...
```yaml
service: victron_vedirect.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: victron_vedirect.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/noahzuch/hass-victron-vedirect/issues) - Report problems
