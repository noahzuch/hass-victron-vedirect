...
This document describes the technical architecture of the Victron VE.Direct custom component for Home Assistant.
...
│   ├── __init__.py          # Exports VictronVEDirectDataUpdateCoordinator
...
│   ├── __init__.py          # Exports VictronVEDirectEntity
...
- `base.py` - Main coordinator class (`VictronVEDirectDataUpdateCoordinator`)
...
**Key class:** `VictronVEDirectDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `VictronVEDirectApiClient`
...
- `VictronVEDirectConfigFlowHandler` (main flow)
- `VictronVEDirectOptionsFlow` (options)
...
**Key class:** `VictronVEDirectEntity` (in `entity/base.py`)
...
2. `VictronVEDirectEntity` for common functionality
...
1. Create directory: `custom_components/victron_vedirect/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `VictronVEDirectEntity`
